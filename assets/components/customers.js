...
export async function renderCustomers() {
    ...
    container.innerHTML = `
        <div class="flex items-end justify-between mb-16 px-2">
            <div>
                 ...
                 <h1 class="text-3xl font-light text-main tracking-tight">Active <span class="font-bold italic text-primary">Clients & Customers.</span></h1>
            </div>
            <div class="flex gap-4">
                <button id="add-client-btn" class="bg-secondary hover:bg-slate-700 text-white px-8 py-4 rounded-xl shadow-sm transition-all flex items-center font-black uppercase tracking-[0.2em] text-[10px]">
                    New Client
                </button>
        ...
        <div class="space-y-16">
            ${ customers.filter(c => ! c.client_id).map(client => {
              const clientCustomers = customers.filter(c => c.client_id === client.id);
              return `
                <div class="space-y-8">
                    <div class="flex items-center gap-4 px-2">
                        <div class="w-2 h-8 rounded-full" style="background-color: ${ client.color }"></div>
                        <h2 class="text-2xl font-bold text-main tracking-tight">${ client.name } <span class="text-[10px] font-black text-dim uppercase tracking-widest ml-2 opacity-40">Client</span></h2>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        ${ clientCustomers.map(c => `
                            <div class="bg-card rounded-2xl p-10 border border-soft shadow-sm group hover:-translate-y-1 transition-all duration-300 relative text-center">
                                ...
                                <div class="flex flex-col items-center pointer-events-none">
                                    <div class="text-[9px] font-black text-primary uppercase tracking-[0.3em] mb-4 opacity-30">Customer</div>
                        ...
                        ${ clientCustomers.length === 0 ? `
                            <div class="col-span-full py-10 opacity-20 text-center border-2 border-dashed border-soft rounded-2xl">
                                <p class="text-[10px] font-black uppercase tracking-[0.4em]">No customers assigned</p>
                            </div>
                        ` : '' }
                    </div>
                </div>
            ...
            ${ customers.some(c => ! c.client_id && ! c.is_client) ? `
                <div class="space-y-8">
                    <h2 class="text-2xl font-bold text-main tracking-tight px-2">Individual Customers</h2>
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        ${ customers.filter(c => ! c.client_id && ! c.is_client).map(c => `
                            <div class="bg-card rounded-2xl p-10 border border-soft shadow-sm group hover:-translate-y-1 transition-all duration-300 relative text-center">
                                <div class="absolute top-6 right-6 opacity-0 group-hover:opacity-100 transition-all duration-300 flex gap-1">
                                    <button class="edit-btn text-dim/50 hover:text-primary transition-colors p-2" data-id="${ c.id }">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
    ...
    modalPortal.innerHTML = `
        <div id="customer-modal" class="fixed inset-0 bg-secondary/40 hidden z-50 backdrop-blur-md pointer-events-auto items-center justify-center overflow-y-auto">
            <div class="min-h-screen w-full flex items-center justify-center p-4">
                <div class="bg-card rounded-2xl shadow-xl w-full max-w-sm p-10 transform transition-all scale-95 opacity-0 text-center relative" id="customer-modal-content">
                    ...
                    <div class="mb-10">
                        ...
                        <p class="text-[10px] font-black text-dim uppercase tracking-[0.3em] mt-2">Details</p>
                    </div>
                    ...
                    <form id="customer-form" class="space-y-6">
                        ...
                        <div class="space-y-2" id="client-select-field">
                            <label class="text-[10px] font-black text-dim uppercase tracking-widest block">Client (Company)</label>
                            <select name="client_id" class="w-full text-center py-4 bg-app border-none rounded-2xl focus:ring-2 focus:ring-primary/20 text-main font-bold appearance-none">
    ...
    const openModal = (data = null, isClient = false) => {
        form.reset();
        form.is_client.value = isClient ? '1' : '0';
        modalPortal.querySelector('#email-field').style.display = isClient ? 'none' : 'block';
        modalPortal.querySelector('#client-select-field').style.display = isClient ? 'none' : 'block';

        if ( data ) {
            modalTitle.textContent = isClient ? 'Edit Client' : 'Edit Customer';
            ...
            modalTitle.textContent = isClient ? 'New Client' : 'New Customer';
            form.id.value = '';
        }
    ...
    container.querySelector('#add-client-btn').onclick = () => openModal(null, true);
    container.querySelector('#add-customer-btn').onclick = () => openModal(null, false);
