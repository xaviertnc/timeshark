...
export async function renderCustomers() {
    ...
    container.innerHTML = `
        <div class="flex items-end justify-between mb-16 px-2">
            <div>
                 ...
                 <h1 class="text-3xl font-light text-slate-800 tracking-tight">Active <span class="font-bold italic text-primary">Customers.</span></h1>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ${customers.map(c => `
                <div class="bg-white rounded-3xl p-10 border border-slate-50 shadow-sm group hover:-translate-y-1 transition-all duration-300 relative text-center">
                    <div class="absolute top-6 right-6 opacity-0 group-hover:opacity-100 transition-all duration-300 flex gap-1">
                        <button class="edit-btn text-slate-100 hover:text-primary transition-colors p-2" data-id="${c.id}">
                    ...
                    <div class="flex flex-col items-center pointer-events-none">
                        ...
                        <p class="text-slate-400 font-medium text-xs truncate opacity-70">${c.email || 'No email'}</p>
                    </div>
                </div>
            `).join('')}
        </div>
    ...
    modalPortal.innerHTML = `
        <div id="customer-modal" class="fixed inset-0 bg-secondary/20 hidden z-50 backdrop-blur-md pointer-events-auto items-center justify-center overflow-y-auto">
            <div class="min-h-screen w-full flex items-center justify-center p-4">
                <div class="bg-white rounded-3xl shadow-xl w-full max-w-sm p-10 transform transition-all scale-95 opacity-0 text-center relative" id="customer-modal-content">
                    <button id="close-modal-btn" class="absolute top-6 right-8 text-slate-300 hover:text-slate-600 text-2xl transition-colors">&times;</button>
                    
                    <div class="mb-10">
                        <h3 class="text-2xl font-bold text-slate-800 tracking-tight" id="modal-title">New Customer</h3>
                        <p class="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em] mt-2">Customer Details</p>
                    </div>
                    ...
                    <form id="customer-form" class="space-y-8">
                        ...
                        <div class="space-y-3">
                            <label class="text-[10px] font-black text-slate-300 uppercase tracking-widest block">Email</label>
                            <input type="email" name="email" placeholder="Enter email" class="w-full text-center py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-primary/20">
                        </div>
                        <div class="pt-6">
