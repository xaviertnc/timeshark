...
export const TaskFilterBar = {
    render(container, filters, options = {}) {
        ...
        let html = FILTER_DEFS.map(f => `
            <button class="task-filter-btn inline-flex items-center gap-1 px-1.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wide leading-none transition-all ${filters[f.key] ? 'bg-primary/20 text-primary' : 'text-dim/50 hover:text-dim hover:bg-white/5'}" data-filter="${f.key}">
        ...
        if (showCompact) {
            html += `
                ...
                <button class="filter-bar-compact-toggle p-1 rounded-md transition-all ${isCompact ? 'bg-primary/20 text-primary' : 'text-dim/50 hover:text-dim hover:bg-white/5'}" title="Toggle Compact Mode">
